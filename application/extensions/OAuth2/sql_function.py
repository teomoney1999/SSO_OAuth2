...
def create_query_client_func(session, client_model): 
    '''
    Create an "query_client" function that can be use for
    AuthorizationServer initiation
    
    :param session: SQLAlchemy session
    :param client_model: Client model class
    '''
    def query_client(client_id): 
        model = session.query(client_model)
        return model.filter_by(client_id=client_id).first()
    return query_client

def create_save_token_func(session, token_model): 
    '''
    Create an "save_token" function that can be use for
    AuthorizationServer initiation
...
def create_query_token_func(session, token_model): 
    ...
    def query_token(token, token_type_hint, client): 
        ...
        q = q.filter_by(
            client_id=client.client_id, revoke=False)
...
def create_bearer_token_validator(session, token_model): 
    ...
    class _BearerTokenValidator(BearerTokenValidator): 
        def authenticate_token(self, token_string): 
            return token_model.query.filter_by(
                access_token=token_string).first()
